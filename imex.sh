...
echo $white"     MNO!!"$red"         [EAGLE]"$red"          MNNOO!    "
...
iptarget() {
	...
	echo $white "╭─"$green"EAGLE@localhost"$cyan" ~/iP-iN-terMUX"$white
	read -p " ╰─$ " target
...
about() {
	sleep 0.15
	echo '''
	Author by       : EagleCyber-74
        version         : 0.1
        made on a date  : 27 - agustus - 2024
        Blog            : https://eaglecyber.vercel.app
        Github          : https://github.com/EagleCyber-74

                        + THANKS TO +

        + ALLAH/IBUK/BAPAK/TEMAN"/ +
